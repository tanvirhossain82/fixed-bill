  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 print:p-0 print:bg-white print:static print:block">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-auto print:max-h-none print:shadow-none print:rounded-none print:max-w-none">
        ...
        {/* Printable bill */}
        <div className="p-6 sm:p-8 print:p-6" id="print-area">
          {/* Company letterhead */}
          <div className="text-center border-b-2 border-emerald-700 pb-4 mb-6">
            <div className="flex items-center justify-center gap-2 mb-1">
              <Fuel className="w-7 h-7 text-emerald-700" />
              <h1 className="text-xl font-bold text-emerald-800">Nippon Paint (Bangladesh) Pvt. Ltd.</h1>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-slate-800">Modhumoti CNG Pump</p>
            </div>
            {bill.preparer && (
              <div>
          ...
          <table className="w-full text-sm border border-slate-300 mb-6">
            <thead>
              <tr className="bg-emerald-50 text-slate-700">
                ...
                <th className="border border-slate-300 px-2 py-2 text-left">{t(lang, 'thDate')}</th>
                <th className="border border-slate-300 px-2 py-2 text-left">{t(lang, 'thFuel')}</th>
                <th className="border border-slate-300 px-2 py-2 text-right">{t(lang, 'thQty')}</th>
                <th className="border border-slate-300 px-2 py-2 text-right">{t(lang, 'thRate')}</th>
                <th className="border border-slate-300 px-2 py-2 text-right">{t(lang, 'thMoney')}</th>
              </tr>
            </thead>
            <tbody>
              {purchases.map((p, i) => (
                <tr key={p.id} className="hover:bg-slate-50">
                  ...
                  <td className="border border-slate-300 px-2 py-1.5">{p.date}</td>
                  <td className="border border-slate-300 px-2 py-1.5">{p.fuel_type}</td>
            ...
            <tfoot>
              <tr className="bg-emerald-100 font-bold">
                <td colSpan={5} className="border border-slate-300 px-2 py-2 text-right">{t(lang, 'billTotal')}</td>
                <td className="border border-slate-300 px-2 py-2 text-right">{fmt(total)}</td>
              </tr>
            </tfoot>
          </table>
          ...
          <div className="mb-6 p-3 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <p className="text-sm font-semibold text-slate-800">{words}</p>
          </div>
